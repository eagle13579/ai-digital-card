...
class TranslationPipeline:
    ...
    def _run_backend(self) -> dict[str, TranslationResult]:
        ...
        for lang in self.target_langs:
            ...
            if self.dry_run:
                # dry-run 模式: 列出待翻译key, 不调用API
                print(f"  [4/4] [预览] 下列 {len(missing_entries)} 个 key 将被翻译 (dry-run):")
                for entry in missing_entries[:20]:
                    key_str = entry.key if hasattr(entry, 'key') else entry.get('key', str(entry)[:60])
                    print(f"    - {key_str}")
                if len(missing_entries) > 20:
                    print(f"    ... 及另外 {len(missing_entries) - 20} 个 key")
