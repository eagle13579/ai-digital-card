...
def get_purpose_template(purpose: str):
    """获取指定用途的推荐模板配置（主题色、页面结构、推荐模块）

    Args:
        purpose: 名片用途标识 (partner/client/investor/supplier)

    Returns:
        该用途的完整模板配置，包括主题色、推荐页面结构和突出模块
    """
...
def list_brochures(
    ...
    if status:
        query = query.filter(Brochure.status == status)
...
def get_brochure_by_share_token(
    share_token: str,
    db: Session = Depends(get_db),
):
    """通过分享token获取画册（公开访问）"""
    brochure = db.query(Brochure).filter(Brochure.share_token == share_token).first()
    if brochure is None:
...
def execute_smart_search(
    ...
    if mode == "hybrid":
        results = MatchEngine.hybrid_search(
            db=db,
        ...
        for other in other_users:
            ...
            tags = db.query(UserTag).filter(UserTag.user_id == other.id).all()
...
def smart_search(
    ...
    三种搜索模式:
    - hybrid:  关键词 + 向量语义混合搜索（默认，推荐）
    - vector:  纯向量语义搜索（TF-IDF + Cosine Similarity）
    - keyword: 纯关键词搜索（子串匹配）

    hybrid模式下: keyword_weight=0.3, vector_weight=0.7

    注意: 本路由注册在 /api/brochures 下，访问路径为 POST /api/brochures/smart-search
    另外提供了 POST /api/brochure/smart-search 别名路由。
    """
