...
# 延迟导入 sync_membership 以避免循环依赖 (crm_router -> auth -> chainke_bridge -> crm)
from app.services import chainke_bridge as _chainke_bridge_lazy
async def _sync_membership(user_id):
    return _chainke_bridge_lazy.sync_membership(user_id)
...
async def register(data: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        await _sync_membership(user.id)
    except Exception:
        pass
