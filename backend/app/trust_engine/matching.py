...
# =============================================================================
# 依据 PRD §4.3.3:
#   Step 0 — 信任预过滤 (在现有6步管线之前)
#   Step 7 — 信任加权修正 (在现有6步管线之后)
#
# 集成: 匹配引擎算法全景_20260602.md 现有6步评分管线
# =============================================================================
...
from app.trust_engine.scoring import ScoreWeights
...
class MatchingConfig:
    """匹配引擎增强配置"""
    # Step 0: 信任预过滤
    MIN_TRUST_FOR_MATCHING: float = 60.0       # 默认最低信任分要求
    LOW_EXPOSURE_THRESHOLD: float = 40.0       # 低于此值降权曝光
    LOW_EXPOSURE_WEIGHT: float = 0.5           # 降权系数

    # Step 7: 信任加权
    TRUST_WEIGHT: float = ScoreWeights.TRUST_WEIGHT   # 0.15
    TOP_TRUST_BOOST: float = ScoreWeights.TOP_TRUST_BOOST  # 1.10
    TOP_TRUST_THRESHOLD: int = ScoreWeights.TOP_TRUST_THRESHOLD  # 90

    # 可配置最低信任要求
    DEFAULT_MIN_TRUST: float = 0.0             # 默认不过滤
...
class PreFilterResult:
    ...
    passed: bool                    # 是否通过过滤
    reason: str                     # 过滤原因
    exposure_weight: float = 1.0    # 曝光权重 (降权用)
...
def pre_filter_candidate(
    ...
    2. 如果信任分 < 40 → 标记为待完善，降低曝光权重50%

    Args:
        trust_score: 候选方的信任评分 [0, 100]
        min_trust_requirement: 需求方的最低信任要求 (None=不过滤)
        config: 匹配配置
...
class PostWeightResult:
    """Step 7 加权修正结果"""
    original_score: float            # 原始匹配分
    adjusted_score: float            # 调整后匹配分
    trust_boost: float               # 信任加速因子 [0.0, 1.0]
    boost_applied: float             # 实际应用的加速倍率
    is_top_boosted: bool = False     # 是否触发顶级信任加成
...
def post_weight_score(
    ...
    在现有6步评分管线之后注入信任加权:
    1. trust_boost = trust_score / 100 → [0.0, 1.0]
    ...
    Args:
        original_match_score: Step 6 输出的原始匹配分
        trust_score: 候选方的信任评分 [0, 100]
        config: 匹配配置
    ...
    """
    cfg = config or MatchingConfig()
    trust_boost = trust_score / 100.0  # [0.0, 1.0]
    ...
    return PostWeightResult(
        ...
        details={
            ...
            "base_multiplier": round(1.0 + trust_boost * cfg.TRUST_WEIGHT, 4) if trust_boost > 0.6 else 1.0,
