...
- network:  已建联的好友可见（基于 connections 表 approved 状态）
- private:  仅自己可见
"""
...
class VisibilityService:
    ...
    async def check_visibility_generic(
        ...
        Args:
            ...
            db: 异步数据库会话（network级别需要查询 connections）
    ...
    async def _is_connected(db: AsyncSession, user_a: int, user_b: int) -> bool:
        ...
        result = await db.execute(
            select(Connection.id).where(
                or_(
                    and_(
                        Connection.user_id == user_a,
                        Connection.contact_id == user_b,
                        Connection.status == "approved",
                    ),
                    and_(
                        Connection.user_id == user_b,
                        Connection.contact_id == user_a,
                        Connection.status == "approved",
                    ),
                )
            )
    ...
    def build_visibility_filter(
        ...
        network_subq = (
            ...
            .where(
                Connection.user_id == viewer_id,
                Connection.status == "approved",
            )
        ).union(
            select(Connection.user_id).where(
                Connection.contact_id == viewer_id,
                Connection.status == "approved",
            )
        )
        ...
        where_clause = (
            ...
            f"         SELECT contact_id FROM connections"
            f"         WHERE user_id = :_viewer_id2 AND status = 'approved'"
            f"     ))"
            f" OR ({table}.visibility = 'network'"
            f"     AND {table}.user_id IN ("
            f"         SELECT user_id FROM connections"
