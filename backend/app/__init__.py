...
def create_app():
    ...
    # ── 远程白泽新增路由 (metrics/health/ai_config/usage/token_pricing) ──
    ...
    # ── 三蛋蛋 Transphee 买家匹配引擎（v1.2.0） ─────────────
    try:
        from app.routers.transphee import router as transphee_router
        app.include_router(transphee_router)
        logger.info("Transphee 买家匹配路由已注册: /api/transphee/*")
    except Exception as e:
        logger.warning("Transphee 路由注册失败（可降级运行）: %s", e)

    # ── K3 Kimi 匹配路由（框架已搭，待 key 激活） ───────────
    try:
        from app.routers.k3_router import router as k3_router
        app.include_router(k3_router)
        logger.info("K3 匹配路由已注册: /api/match/k3/*")
    except Exception as e:
        logger.warning("K3 路由注册失败（可降级运行）: %s", e)
