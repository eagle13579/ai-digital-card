"""
AI数智名片 — 行为序列塔 (BehaviorTower)

行为序列编码模块，使用 Transformer 处理用户行为序列，
生成 L2 归一化的 128d 行为模式嵌入向量。
...
class BehaviorTower(nn.Module):
    ...
    def save(self, path: Union[str, Path]) -> str:
        ...
        logger.info(f"BehaviorTower 模型已保存: {path}")
        return str(path)

    @classmethod
    def load(cls, path: Union[str, Path], **kwargs) -> "BehaviorTower":
        """从文件加载模型权重。
        ...
        model.eval()
        logger.info(f"BehaviorTower 模型已加载: {path}")
        return model
