...
async def _write_memory_db(db_path: str, content: str, category: str = "experience") -> bool:
    ...
    Auto-creates the 'memories' table when the db is empty (0-byte files
    generated by seed_legion_souls have no schema — without this, every
    write returns False and the employee has NO persistent memory).
    """
    import aiosqlite
    ...
    # Self-heal: auto-create parent dir + empty file when missing
    try:
        ...
        logger.warning("Failed to create memory db %s: %s", db_path, exc)
        return False
    ...
    try:
        async with aiosqlite.connect(db_path) as db:
            ...
            # Auto-create schema for empty db (seed-generated 0-byte files)
            if not wrote:
                try:
                    await db.execute(
                    ...
                    logger.warning(
                        "Failed to auto-create memories table in %s: %s",
                        db_path, exc,
                    )
...
class LegionEmployee:
    ...
    def _find_memory_db(self) -> str | None:
        ...
        Returns the path even when the file is missing but the memory/
        directory exists (self-healing: _write_memory_db auto-creates
        the file and schema on first write).
        """
        if not self.emp_dir:
            return None
        memory_dir = f"{self.emp_dir}/memory/"
        db_path = f"{memory_dir}/memory.db"
        if os.path.exists(db_path) or os.path.isdir(memory_dir):
            return db_path
        return None
