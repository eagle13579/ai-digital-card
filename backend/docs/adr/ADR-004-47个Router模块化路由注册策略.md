# ADR-004: 65 个 Router 的模块化路由注册策略
...
链客宝后端有 65 个业务路由模块（Router），涵盖用户、企业、匹配、消息、支付、管理后台等，总计约 314 个 API 端点。路由注册面临选择：
...
3. **`main.py` 中显式 import 所有 65 个 router 并 `app.include_router()`**
...
**负面**:
- 65 个 router 全部 import 导致应用启动时加载所有模块（约 0.5-1s），但可接受
...
| 单一超大 router 文件 | 65 个路由写在一个文件约 7000+ 行，不可维护 |
