...
1. [架构概览](#1-架构概览)
2. [北极星指标](#2-北极星指标)
3. [内容营销自动化引擎](#3-内容营销自动化引擎)
4. [SEO 健康检查面板](#4-seo-健康检查面板)
5. [增长分析 API](#5-增长分析-api)
6. [与现有系统集成](#6-与现有系统集成)
7. [使用场景与工作流](#7-使用场景与工作流)
8. [扩展指南](#8-扩展指南)
9. [API 参考](#9-api-参考)
...
| 项目 | 内容 |
|:-----|:------|
| **北极星指标** | **周活跃匹配数** (Weekly Active Matches, WAM) |
| **英文** | Weekly Active Matches |
| **定义** | 每周至少产生一次「匹配成功」（双方意愿对接、建立联系）的独立匹配对数 |
| **单位** | 对/周 |

### 2.2 为什么是它

北极星指标必须反映产品为用户创造的核心价值 —— 让用户通过 AI 数智名片「高效对接、促成合作」。评估候选指标如下：

| 候选指标 | 评估 | 结论 |
|:---------|:-----|:-----|
| DAU | 反映活跃但不体现匹配价值 | ❌ 非核心 |
| 名片创建数 | 内容生产指标，非结果指标 | ❌ 非核心 |
| **周活跃匹配数** | **直接体现供需对接价值，一指标同时跟踪：匹配引擎质量 + 用户活跃度 + 商务转化** | ✅ **北极星** |
| 匹配成功率 | 比例指标，绝对值增长才是目标 | ❌ 辅助 |
| 付费转化率 | 滞后指标，适合作为健康指标 | ❌ 滞后 |

### 2.3 计算方式

```
周活跃匹配数 = COUNT(DISTINCT match_records)
  WHERE status IN ('connected', 'matched')
    AND matched_at >= NOW() - INTERVAL 7 DAYS
```

- **connected**: 双方已建立联系（互相关注/交换联系方式）
- **matched**: 匹配引擎推荐且双方已查看对方
...
| 健康指标 | 说明 | 预警阈值 |
|:---------|:-----|:---------|
| 匹配满意率 | 用户对匹配结果的反馈评分 (1-5) | < 3.5 需干预 |
| 匹配响应率 | 收到推荐后 24h 内查看/回复比例 | < 30% 需优化 |
| 周活跃匹配用户数 | 发起匹配的独立用户数 | 环比下降 > 20% 预警 |
| 平均匹配分 | 匹配引擎计算的平均匹配得分 | < 0.4 需调优 |

### 2.5 数据来源

- 主表: `match_records`
- 关联表: `users`, `user_tags`
- 分析 API: `GET /api/v1/growth/metrics` → `total_matches`
- 趋势追踪: `GET /api/v1/growth/trends`
