...
class TestHealth:
    async def test_health_endpoint(self, client):
        """GET /health should return 200 with 'OK' plain text."""
...
class TestBrochure:
    ...
    async def test_create_brochure(self, client, auth_headers):
        ...
        resp = await client.post(self.BASE_URL, json=payload, headers=auth_headers)
        assert resp.status_code in (200, 201)
        data = resp.json()
    ...
    async def test_create_brochure_without_auth(self, client):
        """Creating a brochure without auth should return 401."""
        payload = {"title": "无权限名片", "pages": []}
        resp = await client.post(self.BASE_URL, json=payload)
        assert resp.status_code == status.HTTP_401_UNAUTHORIZED

    async def test_list_brochures(self, client, auth_headers, test_user, test_db):
        ...
        resp = await client.get(self.BASE_URL, headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert isinstance(data, list)
        assert any(b["title"] == "测试列表" for b in data)

    async def test_get_brochure_by_id(self, client, auth_headers, test_user, test_db):
        ...
        resp = await client.get(f"{self.BASE_URL}/{brochure.id}", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
    ...
    async def test_get_nonexistent_brochure(self, client, auth_headers):
        ...
        resp = await client.get(f"{self.BASE_URL}/99999", headers=auth_headers)
        assert resp.status_code == status.HTTP_404_NOT_FOUND

    async def test_delete_brochure(self, client, auth_headers, test_user, test_db):
        ...
        resp = await client.delete(f"{self.BASE_URL}/{brochure.id}", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK

        # Verify it's gone
        resp2 = await client.get(f"{self.BASE_URL}/{brochure.id}", headers=auth_headers)
...
class TestBrochurePublishShare:
    ...
    async def test_publish_brochure(self, client, auth_headers, test_user, test_db):
        ...
        resp = await client.post(f"{self.BASE_URL}/{brochure.id}/publish", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["status"] == "published"
        assert len(data["share_token"]) > 0

    async def test_publish_others_brochure(self, client, auth_headers, test_db, second_user):
    ...
    async def test_get_shared_brochure(self, client, test_user, test_db):
        ...
        resp = await client.get(f"{self.BASE_URL}/share/testsharetoken123")
        assert resp.status_code == status.HTTP_200_OK
        assert resp.json()["title"] == "分享测试"

    async def test_get_shared_brochure_invalid_token(self, client):
        """GET /api/brochures/share/{invalid_token} should return 404."""
        resp = await client.get(f"{self.BASE_URL}/share/nonexistenttoken")
        assert resp.status_code == status.HTTP_404_NOT_FOUND
...
class TestTags:
    ...
    async def test_get_my_tags_empty(self, client, auth_headers):
        """GET /api/tags/me should return empty lists initially."""
        resp = await client.get(f"{self.BASE_URL}/me", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["provide"] == []
        assert data["need"] == []

    async def test_add_tags(self, client, auth_headers):
        ...
        resp = await client.post(f"{self.BASE_URL}/me", json=payload, headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert len(data) == 2
        assert data[0]["tag_type"] == "provide"

    async def test_add_and_retrieve_tags(self, client, auth_headers):
        ...
        await client.post(f"{self.BASE_URL}/me", json=payload, headers=auth_headers)
    ...
    async def test_delete_tag(self, client, auth_headers, test_user, test_db):
        ...
        resp = await client.delete(f"{self.BASE_URL}/me/{tag.id}", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK

        # Verify deletion
        resp2 = await client.get(f"{self.BASE_URL}/me", headers=auth_headers)
        assert resp2.json()["provide"] == []

    async def test_get_user_tags(self, client, test_user, test_db):
        ...
        resp = await client.get(f"{self.BASE_URL}/users/{test_user.id}")
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
...
class TestTrustNetwork:
    ...
    async def test_get_empty_network(self, client, auth_headers):
        """GET /api/trust/network should return empty lists."""
        resp = await client.get(f"{self.BASE_URL}/network", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["trusting"] == []
        assert data["trusted_by"] == []

    async def test_add_trust(self, client, auth_headers, test_db, second_user):
        """POST /api/trust/network should create trust relationship."""
        payload = {"trusted_user_id": second_user.id}
        resp = await client.post(f"{self.BASE_URL}/network", json=payload, headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["trusted_user_id"] == second_user.id

    async def test_add_trust_self(self, client, auth_headers, test_user):
        """Adding trust to self should return 400."""
        payload = {"trusted_user_id": test_user.id}
        resp = await client.post(f"{self.BASE_URL}/network", json=payload, headers=auth_headers)
        assert resp.status_code == status.HTTP_400_BAD_REQUEST

    async def test_add_duplicate_trust(self, client, auth_headers, test_db, second_user):
    ...
    async def test_remove_trust(self, client, auth_headers, test_db, second_user):
        ...
        resp = await client.delete(f"{self.BASE_URL}/network/{second_user.id}", headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK

        resp2 = await client.get(f"{self.BASE_URL}/network", headers=auth_headers)
        assert resp2.json()["trusting"] == []

    async def test_trust_network_endpoint_without_auth(self, client):
        """Trust endpoints should require authentication."""
        resp = await client.get(f"{self.BASE_URL}/network")
        assert resp.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestUserProfile:
    ...
    async def test_get_my_profile(self, client, auth_headers, test_user):
    ...
    async def test_update_profile(self, client, auth_headers):
        ...
        resp = await client.put(f"{self.BASE_URL}/me", json=payload, headers=auth_headers)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["name"] == "新名字"
        assert data["title"] == "高级工程师"

    async def test_get_user_by_id(self, client, test_user, test_db):
        """GET /api/users/{user_id} should return user info (public)."""
        resp = await client.get(f"{self.BASE_URL}/{test_user.id}")
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data["name"] == test_user.name
