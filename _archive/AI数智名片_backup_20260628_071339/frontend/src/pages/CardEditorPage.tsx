...
export default function CardEditorPage() {
  ...
  const handleDeleteCard = async (id: number) => {
    if (!window.confirm(t('确定要删除这张AI数智名片吗？此操作不可恢复。'))) return;
    setLoading(true);
    try {
      const res = await api.delete(`/api/v1/brochures/${id}`);
      if (res.code === 200) {
        showToast(t('名片已删除'), 'success');
        navigate('/');
      } else {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setError(t('不支持的文件格式。请上传 PDF 或图片文件（JPG/PNG/BMP/WebP）'));
      return;
    }
  ...
  const handleShowQR = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('获取二维码失败'));
  ...
  const handleDownloadQR = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('下载二维码失败'));
  ...
  const handleApplyCopy = (purpose: string, content: string) => {
    if (purpose === 'bio') {
      ...
      showToast(t('标语已生成: {content}', { content }), 'success');
    } else {
      showToast(`${purpose === 'company' ? t('公司介绍') : t('推荐语')}${t('已生成')}`, 'success');
    }
  ...
  const renderUpload = () => (
    <div className="space-y-6 max-w-lg mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-on-surface">{t('AI 名片扫描')}</h2>
          <p className="text-xs text-text-muted">{t('上传名片图片或 PDF，AI 自动提取信息')}</p>
        </div>
      </div>
      ...
      >
        ...
        {loading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-text-muted">{t('正在识别名片文字...')}</p>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-on-surface">{t('点击上传或拖拽文件到此处')}</p>
              <p className="text-xs text-text-muted mt-1">{t('支持 PDF、JPG、PNG、BMP、WebP 格式')}</p>
            </div>
          </div>
        )}
      </div>
  ...
  const renderReview = () => (
    <div className="space-y-6 max-w-lg mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-on-surface">{t('确认名片信息')}</h2>
          <p className="text-xs text-text-muted">{t('请核对 AI 提取的信息，可手动修改')}</p>
        </div>
      </div>
      ...
      {rawText && (
        <details className="bg-slate-50 rounded-xl p-3">
          <summary className="text-xs text-text-muted cursor-pointer select-none">{t('OCR 原始识别文字')}</summary>
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FieldInput icon={<User className="w-4 h-4" />} label={t('姓名 *')} value={fields.name || ''} onChange={(v) => updateField('name', v)} placeholder={t('请输入姓名')} />
          <FieldInput icon={<Briefcase className="w-4 h-4" />} label={t('职位')} value={fields.position || ''} onChange={(v) => updateField('position', v)} placeholder={t('请输入职位')} />
        </div>
        <FieldInput icon={<Building2 className="w-4 h-4" />} label={t('公司')} value={fields.company || ''} onChange={(v) => updateField('company', v)} placeholder={t('请输入公司名称')} />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FieldInput icon={<Phone className="w-4 h-4" />} label={t('手机')} value={fields.phone || ''} onChange={(v) => updateField('phone', v)} placeholder={t('请输入手机号')} type="tel" />
          <FieldInput icon={<Mail className="w-4 h-4" />} label={t('邮箱')} value={fields.email || ''} onChange={(v) => updateField('email', v)} placeholder={t('请输入邮箱')} type="email" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FieldInput icon={<MessageCircle className="w-4 h-4" />} label={t('微信')} value={fields.wechat || ''} onChange={(v) => updateField('wechat', v)} placeholder={t('请输入微信号')} />
          <FieldInput icon={<Globe className="w-4 h-4" />} label={t('官网')} value={fields.website || ''} onChange={(v) => updateField('website', v)} placeholder={t('请输入网址')} />
        </div>
        <FieldInput icon={<MapPin className="w-4 h-4" />} label={t('地址')} value={fields.address || ''} onChange={(v) => updateField('address', v)} placeholder={t('请输入地址')} />
      </div>
      ...
      <div className="flex gap-3">
        <button onClick={handleOpenAIAssistant} className="py-3 px-3 rounded-xl border border-border-light text-primary font-medium text-sm hover:bg-primary/5 transition-colors flex items-center justify-center gap-2">
          <Sparkles className="w-4 h-4" /> {t('AI 写作')}
        </button>
        ...
        <button onClick={handleProceedToTemplate} disabled={!fields.name?.trim()} className="flex-1 py-3 px-4 rounded-xl bg-primary text-white font-medium text-sm hover:bg-primary-container transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          <Palette className="w-4 h-4" /> {t('选择模板')}
        </button>
      </div>
    </div>
  );
  ...
  const renderTemplate = () => (
    <div className="space-y-6 max-w-lg mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-on-surface">{t('选择名片模板')}</h2>
          <p className="text-xs text-text-muted">{t('为您的AI数智名片选择一套风格')}</p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className={`aspect-[3/4] ${preview.bg} p-3 flex flex-col justify-between`}>
                ...
                <div>
                  <p className={`text-sm font-bold ${preview.text} truncate`}>{fields.name || t('姓名')}</p>
                  <p className={`text-[10px] ${preview.text} opacity-70 truncate`}>{fields.position || t('职位')}</p>
                </div>
              ...
              <div className="p-2 text-center bg-white">
                <p className="text-xs font-medium text-on-surface">{t(tpl.name)}</p>
              </div>
            </button>
          );
        })}
      ...
      >
        {loading ? (
          <><Loader2 className="w-4 h-4 animate-spin" /> {t('生成中...')}</>
        ) : (
          <><Sparkles className="w-4 h-4" /> {t('生成AI数智名片')}</>
        )}
      </button>
    </div>
  );
    ...
    return (
      <div className="space-y-6 max-w-lg mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-on-surface">{t('AI数智名片')}</h2>
            <p className="text-xs text-text-muted">{cardData.name} · {t('浏览 {n} 次', { n: cardData.view_count })}</p>
          </div>
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-text-muted mt-2">{t('第 {current} 页，共 {total} 页', { current: currentPage + 1, total: totalPages })}</p>
        </div>

        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="flex-1 bg-slate-50 rounded-xl px-3 py-2.5 text-xs text-on-surface truncate">{shareUrl}</div>
            <button onClick={handleCopyLink} className="p-2.5 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors" title={copied ? t('已复制') : t('复制链接')}>
        ...
        {matchResults.length > 0 && (
          <div className="border-t border-border-light pt-4">
            <h3 className="text-sm font-bold text-on-surface mb-3 flex items-center gap-2">
              ...
              {t('AI 匹配结果 ({n})', { n: matchResults.length })}
            </h3>
            <div className="space-y-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className={`text-xs font-medium ${item.score >= 0.7 ? 'text-green-600' : item.score >= 0.4 ? 'text-amber-600' : 'text-text-muted'}`}>
                        {t('匹配度 {n}%', { n: Math.round(item.score * 100) })}
                      </span>
                    </div>
                  </div>
              ...
              {matchResults.length > 5 && (
                <p className="text-xs text-primary text-center py-2">{t('还有 {n} 个匹配结果', { n: matchResults.length - 5 })}</p>
              )}
            </div>
          </div>
        )}
    ...
    if (loading && !cardData) {
      return (
        <div className="flex flex-col items-center py-20 gap-3">
          ...
          <p className="text-sm text-text-muted">{t('加载名片详情...')}</p>
        </div>
      );
    }
    ...
    if (!cardData) {
      return (
        <div className="text-center py-20">
          <p className="text-sm text-text-muted">{t('名片不存在或已被删除')}</p>
          <button onClick={() => navigate('/')} className="mt-4 text-sm text-primary underline">{t('返回列表')}</button>
        </div>
      );
    }
      ...
      <div className="space-y-6 max-w-lg mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-bold text-on-surface">{t('名片详情')}</h2>
          </div>
          <button onClick={() => handleDeleteCard(cardData.id)} disabled={loading}
            className="p-2 rounded-lg text-rose-500 hover:bg-rose-50 transition-colors disabled:opacity-50" title={t('删除名片')}
          >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2 text-xs text-text-muted">
                <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> {t('浏览 {n} 次', { n: cardData.view_count ?? 0 })}</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <Share2 className="w-4 h-4" /> {t('分享名片')}
          </button>
          ...
          >
            <Sparkles className="w-4 h-4" /> {t('编辑名片')}
          </button>
        </div>
        ...
        >
          <Sparkles className="w-4 h-4" /> {t('AI 名片助手（写作 & 优化）')}
        </button>
        ...
        >
          <Trash2 className="w-4 h-4" /> {t('删除名片')}
        </button>
        ...
        <div className="bg-white rounded-2xl p-4 border border-border-light">
          ...
          {trustNetworkLoading ? (
            ...
            <p className="text-xs text-text-muted text-center py-4">{t('暂无信任用户')}</p>
          ) : (
            <div className="space-y-2">
              {trustNetwork.map((user) => (
                <div key={user.id} className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
                  ...
                  <button onClick={() => handleRemoveTrust(user.id)} className="text-xs text-rose-500 hover:text-rose-700 transition-colors shrink-0 ml-2">{t('移除')}</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl p-4 border border-border-light">
          ...
          {visitorsLoading ? (
            ...
            <p className="text-xs text-text-muted text-center py-4">{t('暂无访客记录')}</p>
          ) : (
              ...
              {visitors.map((v) => (
                <div key={v.id} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3">
                  <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 text-xs font-bold shrink-0">
                    {(v.visitor_name || t('访'))[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-on-surface truncate">{v.visitor_name || t('匿名访客')}</p>
        ...
        return (
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 space-y-3">
              ...
              {(!page.fields || page.fields.length === 0) && <p className="text-sm opacity-60">{t('暂无联系方式')}</p>}
            </div>
          </div>
        );
      case 'company':
        return (
          <div className="flex flex-col h-full">
            <h3 className="text-base font-bold mb-4" style={{ color: page.style.accentColor }}>{page.title}</h3>
            <div className="flex-1 space-y-3">
              {page.content?.company && (<div><p className="text-xs opacity-60 mb-0.5">{t('公司')}</p><p className="text-sm font-medium">{page.content.company}</p></div>)}
              {page.content?.position && (<div><p className="text-xs opacity-60 mb-0.5">{t('职位')}</p><p className="text-sm font-medium">{page.content.position}</p></div>)}
              {page.content?.address && (<div><p className="text-xs opacity-60 mb-0.5">{t('地址')}</p><p className="text-sm">{page.content.address}</p></div>)}
              {page.content?.website && (<div><p className="text-xs opacity-60 mb-0.5">{t('官网')}</p><p className="text-sm text-blue-600 truncate">{page.content.website}</p></div>)}
            </div>
          </div>
        );
      case 'qrcode':
    ...
    <div className="min-h-screen">
      ...
      {error && (
        <div className="mb-4 bg-rose-50 border border-rose-200 rounded-xl p-3 text-xs text-rose-700 flex items-center justify-between max-w-lg mx-auto">
          <span>{error}</span>
          <button onClick={() => setError('')} className="ml-2 underline">{t('关闭')}</button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={handleCloseQR}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-on-surface">{t('名片二维码')}</h3>
            ...
            <div className="flex items-center justify-center py-4">
              {qrLoading ? (
                ...
                <p className="text-xs text-text-muted">{t('二维码加载失败')}</p>
              )}
            </div>
            ...
            >
              <Download className="w-4 h-4" /> {t('保存二维码')}
            </button>
          </div>
        </div>
      )}
