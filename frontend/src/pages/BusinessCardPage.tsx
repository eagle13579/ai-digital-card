  ...
  const fetchCardList = useCallback(async () => {
    ...
    try {
      ...
      if (res.code === 200 && Array.isArray(res.data)) {
        ...
        setCardList((res.data as { items: CardListItem[] }).items as CardListItem[]);
      } else {
        setCardList([]);
      }
    } catch (e: any) {
  ...
  const renderList = () => (
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-on-surface">我的AI数智名片</h2>
        <p className="text-sm text-text-muted mt-1">
          共 {cardList.length} 张名片
        </p>
      </div>
      ...
      {listLoading ? (
        <div className="flex flex-col items-center py-12 gap-3">
          ...
          <p className="text-sm text-text-muted">加载名片列表...</p>
        </div>
      ) : cardList.length === 0 ? (
        <div className="flex flex-col items-center py-12 gap-4">
          ...
          <div className="text-center">
            <p className="text-base font-medium text-on-surface">暂无AI数智名片</p>
            <p className="text-sm text-text-muted mt-1">点击下方按钮创建您的第一张名片</p>
          </div>
        </div>
      ) : (
