...
API 端点概要:
    GET    /api/v1/brochures              — 获取所有画册
  GET    /api/v1/brochures/{user_id}    — 获取指定用户画册
  POST   /api/v1/brochures              — 创建画册
  PUT    /api/v1/brochures/{user_id}    — 更新画册
  DELETE /api/v1/brochures/{user_id}    — 删除画册
  GET    /api/v1/brochures/{user_id}/trust_network   — 获取信任网络
  POST   /api/v1/brochures/{user_id}/trust_network   — 添加信任关系
  DELETE /api/v1/brochures/{user_id}/trust_network   — 移除信任关系
  GET    /api/v1/brochures/{user_id}/matches         — 获取匹配列表
  POST   /api/v1/match                               — 匹配引擎
  GET    /api/v1/users                               — 获取用户列表
  GET    /api/v1/users/{user_id}                     — 获取指定用户
  GET    /api/v1/health                              — 健康检查
  旧路径 /api/* 自动 307 重定向到 /api/v1/*
"""
...
# ── 国际化 i18n ──────────────────────────────────────────
from backend.app.i18n import t as _t, detect_lang

# 当前请求的语言上下文
_current_lang_var: contextvars.ContextVar[str] = contextvars.ContextVar("current_lang", default="zh")

def get_current_lang() -> str:
    """获取当前请求的语言。"""
    return _current_lang_var.get()

def _(key: str) -> str:
    """简写翻译函数，从当前请求上下文自动获取语言。"""
    return _t(key, get_current_lang())

# 翻译辅助常量（保持迁移时能搜索到原有中文）
# 使用函数调用: _("auth_token_invalid")
...
HASH_SALT = "digital_brochure_v2"  # 固定盐，用于 hashlib SHA256


def _hash_password(password: str) -> str:
    """使用 SHA256 + 固定盐哈希密码。"""
    return hashlib.sha256(f"{HASH_SALT}:{password}".encode()).hexdigest()
...
# ── i18n 语言检测中间件 ────────────────────────────────
@app.middleware("http")
async def i18n_language_middleware(request: Request, call_next):
    """根据 Accept-Language 请求头自动检测语言并设置上下文。"""
    lang = detect_lang(request.headers.get("Accept-Language", ""))
    _current_lang_var.set(lang)
    response = await call_next(request)
    return response
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if not allowed:
        ...
        return JSONResponse(
            status_code=429,
            content={
                "detail": _("rate_limit_exceeded"),
                "message": _("rate_limit_exceeded"),
...
async def api_redirect_middleware(request: Request, call_next):
    """将旧 /api/* 路径 307 重定向到 /api/v1/*（除 /api/health 和 / 外）。"""
    path = request.url.path
    # 已带版本前缀、健康检查、首页 — 直接放行
    if path.startswith("/api/v1/") or path == "/api/health" or path == "/":
        response = await call_next(request)
        return response
...
@v1_router.post("/auth/register", status_code=201)
def register(data: RegisterRequest):
    """用户注册（手机号 + 密码 + 姓名）。"""
    # 检查手机号是否已注册
    existing = db_get_user_by_phone(data.phone)
    if existing:
        raise HTTPException(status_code=409, detail=_("auth_phone_exists"))

    # 创建用户
    user = db_create_user(name=data.name, phone=data.phone, password=data.password)
    if not user:
        raise HTTPException(status_code=500, detail=_("auth_register_failed"))

    # 自动登录：创建 token
    token = db_create_token(user["user_id"])
    if not token:
        raise HTTPException(status_code=500, detail=_("auth_token_create_failed"))
...
def login(data: LoginRequest):
    ...
    if not user:
        raise HTTPException(status_code=401, detail=_("auth_login_failed"))
...
def get_me(current_user_id: str = Depends(require_user_id)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail=_("auth_user_not_found"))
    return {
        "code": 200,
...
def logout(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)):
    ...
    return {"code": 200, "message": _("auth_logout_success")}


# ── 我的画册列表（当前用户） ─────────────────────────

@v1_router.get("/brochure/list")
...
@v1_router.get("/brochures/{user_id:path}")
def get_brochure(user_id: str):
    ...
    if not brochure:
        ...
        if brochure:
            ...
            raise HTTPException(status_code=404, detail=_("brochure_not_found"))
    return {"code": 200, "data": brochure}


# ── 4. 创建画册（需登录） ─────────────────────────────

@v1_router.post("/brochures", status_code=201)
def create_brochure(data: BrochureCreate, current_user_id: str = Depends(require_user_id)):
    ...
    if effective_user_id != current_user_id:
        raise HTTPException(status_code=403, detail=_("brochure_no_permission_create"))

    if bid in BROCHURES:
        raise HTTPException(status_code=409, detail=_("brochure_already_exists"))
    ...
    return {"code": 201, "message": _("brochure_created"), "data": brochure}


# ── 5. 更新画册（需登录 + 所有权校验） ────────────────

@v1_router.put("/brochures/{user_id:path}")
def update_brochure(user_id: str, data: BrochureUpdate,
    ...
    if brochure.get("user_id") != current_user_id:
        raise HTTPException(status_code=403, detail=_("brochure_no_permission_update"))
    ...
    return {"code": 200, "message": _("brochure_updated"), "data": brochure}


# ── 6. 删除画册（需登录 + 所有权校验） ────────────────

@v1_router.delete("/brochures/{user_id:path}")
def delete_brochure(user_id: str, current_user_id: str = Depends(require_user_id)):
    ...
    # 用户隔离：只能删除自己的画册
    if brochure.get("user_id") != current_user_id:
        raise HTTPException(status_code=403, detail=_("brochure_no_permission_delete"))

    del BROCHURES[bid]
    db_delete_brochure(bid)

    return {"code": 200, "message": _("brochure_deleted")}


# ── 7. 获取信任网络 ──────────────────────────────────

@v1_router.get("/brochures/{user_id:path}/trust_network")
...
@v1_router.post("/brochures/{user_id:path}/trust_network")
def add_trust(user_id: str, data: TrustAddRequest,
              current_user_id: str = Depends(require_user_id)):
    # 用户隔离：只能操作自己的信任网络
    if user_id != current_user_id:
        raise HTTPException(status_code=403, detail=_("trust_no_permission"))
    bid = ensure_brochure_id(user_id)
    ...
    if trusted_bid not in BROCHURES:
        raise HTTPException(status_code=404, detail=_("trust_target_not_found"))

    ok = db_add_trust(user_id, data.trusted_user_id)
    if not ok:
        raise HTTPException(status_code=500, detail=_("trust_add_failed"))
    ...
    return {"code": 200, "message": _("trust_added")}


# ── 9. 移除信任关系（需登录 + 只能操作自己的信任网络） ─

@v1_router.delete("/brochures/{user_id:path}/trust_network")
def remove_trust(user_id: str, trusted_user_id: str = Query(..., description="被信任的用户ID"),
    ...
    if not ok:
        raise HTTPException(status_code=500, detail=_("trust_remove_failed"))
    ...
    return {"code": 200, "message": _("trust_removed")}


# ── 10. 获取匹配列表 ─────────────────────────────────

@v1_router.get("/brochures/{user_id:path}/matches")
...
def match_brochures(req: MatchRequest):
    ...
    if source_id not in BROCHURES:
        raise HTTPException(status_code=404, detail=_("match_source_not_found"))
...
@v1_router.get("/users/{user_id:path}")
...
@v1_router.get("/brochure/{brochure_id}/brochure", response_class=HTMLResponse)
...
@v1_router.get("/brochure/{brochure_id}/visitors")
...
@v1_router.get("/brochure/{brochure_id}/qrcode")
...
def scan_redirect(brochure_id: str, request: Request):
    ...
    scheme = request.headers.get("x-forwarded-proto", "http")
    host = request.headers.get("host", f"localhost:{PORT}")
    preview_url = f"{scheme}://{host}/api/v1/brochure/{bid}/brochure?source=scan"
    return RedirectResponse(url=preview_url, status_code=302)


# ── 19. 链客宝数据同步 ──────────────────────────────────

@v1_router.get("/brochure/sync/chainke")
def sync_chainke(current_user_id: str = Depends(require_user_id)):
    ...
    if not bridge:
        return {
            "code": 0,
            "message": _("chainke_module_not_loaded"),
            "data": {
                "trust_pushed": 0,
                "matches_pulled": 0,
                "source": "none",
    ...
    return {
        "code": 0,
        "message": _("chainke_sync_complete"),
        "data": {
...
@v1_router.post("/brochure/batch-import", status_code=201)
def batch_import(data: BatchImportRequest):
    """批量导入企业用户，自动创建画册。"""
    if not data.users:
        raise HTTPException(status_code=400, detail=_("batch_import_empty"))
    ...
    for idx, item in enumerate(data.users):
        try:
            ...
            if ok:
                ...
                errors.append({"name": item.name, "error": _("batch_import_db_error")})
        except Exception as e:
